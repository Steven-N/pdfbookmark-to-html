...
def pdf_bookmarks_to_html(root_dir, output_path, output_filename):
    ...
    # Loop through all the PDF files in the directory and retrieve all bookmarks.
    ...
    final_bookmarks = {}
    # Loop through all bookmarks and create a dictionary with the path, bookmark title, and page number
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Extract bookmarks from PDF files and generate an HTML file.')
    parser.add_argument('--input-dir', help='The directory containing the input PDF files.')
    parser.add_argument('--output-dir', help='The output path.')
    parser.add_argument('--output-filename', help='The output filename.', default='bookmarks.html', required=False)
    ...
    if not os.path.exists(root_dir):
        raise RuntimeError(f"The directory {root_dir} does not exist. Please create it and add PDF files to it.")
